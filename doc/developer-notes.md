...
TOUR Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 5982.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 5983 for connections and services them.
...
However, in open source software such as TOUR Core, where everyone uses
...
pay attention to for reviewers of TOUR Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: TOUR Core uses tinyformat, which is type safe. Leave them out to avoid confusion
