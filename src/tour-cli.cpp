...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("TOUR Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  tour-cli [options] <command> [params]  " + _("Send command to TOUR Core") + "\n" +
                  "  tour-cli [options] help                " + _("List commands") + "\n" +
                  "  tour-cli [options] help <command>      " + _("Get help for a command") + "\n";
