...
const char * const BITCOIN_CONF_FILENAME = "tour.conf";
const char * const BITCOIN_PID_FILENAME = "tourd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "tour" is a composite category enabling all TOUR-related debug output
            if(ptrCategory->count(string("tour"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TOUR
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TOUR
    // Mac: ~/Library/Application Support/TOUR
    // Unix: ~/.tour
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TOUR";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TOUR";
#else
    // Unix
    return pathRet / ".tour";
#endif
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty tour.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override tour.conf
