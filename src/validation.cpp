...
#if defined(NDEBUG)
# error "TOUR Core cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in nodes) are considered zero fee (for relaying, mining and transaction creation) */
...
const string strMessageMagic = "TOUR Signed Message:\n";
...
{
    CAmount ret = (blockValue/100) * 75; // 75%
...
void CheckForkWarningConditions()
{
    ...
    // If our best fork is no longer within 72 blocks (+/- 2.5 hours if no one mines it)
    // of our head, drop it
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // TOUR : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(TOUR): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(TOUR): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // TOUR : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(TOUR): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(TOUR): spork is off, skipping transaction locking checks\n");
    }
